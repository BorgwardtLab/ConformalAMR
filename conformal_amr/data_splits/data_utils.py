...
class DataSplitter:
    ...
    def __init__(self, long_table=None, dataset=None):
        ...
        if dataset is not None:
            self.long_table = long_table[
                long_table["dataset"].isin(list(dataset))
    ...
    def sample_id_train_test_split(
        self,
        test_size=0.2,
        random_state=42,
        select_species=None,
        reverse_sp_selection=False,
    ):
        # Initialize lists to store final train and test splits
        train_X_list = []
        test_X_list = []

        # Group by species
        for species, group in self.long_table.groupby("species"):
            # Extract unique sample IDs within each species
            sample_ids = group["sample_id"].unique()

            # Drop duplicates by sample_id to get the corresponding response
            stratify_labels = group.drop_duplicates("sample_id")["response"]
            if select_species is not None:
                if not reverse_sp_selection and species not in select_species:
                    train_X_list.append(sample_ids)
                    continue
                elif reverse_sp_selection and species in select_species:
                    train_X_list.append(sample_ids)
                    continue
            ...
            # Collect train and test rows based on the sample_id split
            train_X_list.append(train_patients)
            test_X_list.append(test_patients)

        # Concatenate results to get the final train and test DataFrames
        train_X = np.concatenate(train_X_list)
        test_X = np.concatenate(test_X_list) if len(test_X_list) > 0 else pd.DataFrame()

        # Filter data based on the sample_id split
        train_X = self.long_table.loc[
            self.long_table["sample_id"].isin(train_X)
        ].reset_index(drop=True)
        test_X = self.long_table.loc[
            self.long_table["sample_id"].isin(test_X)
        ].reset_index(drop=True)
