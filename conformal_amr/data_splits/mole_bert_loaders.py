...
def mol_to_graph_data_obj_simple(mols):
    """
    Converts a list of rdkit mol objects to a single graph Data object required
    by the pytorch geometric package. Each molecule will be treated as a separate
    ...
    # Initialize lists to store atom and bond features from all molecules
    all_atom_features = []
    all_edges_list = []
    all_edge_features_list = []

    # To keep track of the cumulative number of atoms, so that bonds between different
    # molecules aren't created by accident. Each molecule's atoms will have different indices.
    cum_atoms = 0
    ...
    for mol in mols:
        ...
        # Add current molecule's atom features to the global list
        all_atom_features.extend(atom_features_list)
        ...
        if len(mol.GetBonds()) > 0:  # mol has bonds
            ...
            for bond in mol.GetBonds():
                i = bond.GetBeginAtomIdx() + cum_atoms
                j = bond.GetEndAtomIdx() + cum_atoms
                edge_feature = [
            ...
            # Append to global edge lists
            all_edges_list.extend(edges_list)
            all_edge_features_list.extend(edge_features_list)

        # Update cumulative atom count to shift indices for the next molecule
        cum_atoms += mol.GetNumAtoms()

    # Convert atom features and bond features into tensors
    x = torch.tensor(np.array(all_atom_features), dtype=torch.long)

    if len(all_edges_list) > 0:
        edge_index = torch.tensor(np.array(all_edges_list).T, dtype=torch.long)
        edge_attr = torch.tensor(np.array(all_edge_features_list), dtype=torch.long)
    else:
...
class MoleculeGraphDataset(InMemoryDataset):
    def __init__(self, mol_list_of_lists, root=".", transform=None, pre_transform=None):
        """
        Initializes the MoleculeGraphDataset.

        :param mol_list_of_lists: List of lists of RDKit molecule objects.
                                  Each sublist will be converted into a single graph
                                  where each molecule is a separate connected component.
        :param root: Root directory (required by InMemoryDataset, but unused here).
        :param transform: Optional transform applied to each graph.
        :param pre_transform: Optional pre-transform applied before storing data.
        """
        self.mol_list_of_lists = mol_list_of_lists
        super(MoleculeGraphDataset, self).__init__(root, transform, pre_transform)

        # Process molecules and create graphs
        self.data, self.slices = self.process_molecules()

    def process_molecules(self):
        """
        Process the list of molecule lists and converts them into a list of graph Data objects.
        Each element in the list (a sublist of molecules) will be converted into a graph.

        :return: Processed data and slice information as required by InMemoryDataset.
        """
        data_list = []

        for mol_list in self.mol_list_of_lists:
            graph_data = mol_to_graph_data_obj_simple(
                mol_list
            )  # Convert each sublist to a graph
            data_list.append(graph_data)

        if self.pre_transform is not None:
            data_list = [self.pre_transform(data) for data in data_list]

        # Collate the data into a format expected by PyTorch Geometric
        data, slices = self.collate(data_list)
        return data, slices

    def get(self, idx):
        """
        Get the graph Data object corresponding to the index idx.
        """
        data = Data()
        for key in self.data.keys():
            item, slices = self.data[key], self.slices[key]
    ...
    def len(self):
        """
        Return the number of graphs (which is the number of sublists in the original input).
        """
        return len(self.mol_list_of_lists)
...
class DataLoaderMaskingPred(torch.utils.data.DataLoader):
    ...
    def collate_fn(self, batches):
        ...
        return BatchMasking.from_data_list(batches)
