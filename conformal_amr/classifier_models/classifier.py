...
from .modules import Conv1d_Block, MLP_Block, ResMLP
...
class Residual_AMR_Classifier(nn.Module):
    ...
    def __init__(self, config):
        ...
        # Sample embedding
        if (
            config["species_embedding_dim"] == 0
            and config["conv_out_size"] == config["sample_embedding_dim"]
        ):
...
class SpeciesBaseline_ResAMR_Classifier(Residual_AMR_Classifier):
    ...
    def forward(self, batch):
        species_idx, x_spectrum, dr_tensor, response, dataset = batch
        species_tensor = (
            F.one_hot(species_idx, num_classes=self.n_species).squeeze().float()
        )
